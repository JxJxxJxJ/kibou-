...
## Aliases
##
## auto ls after cd with --exa colors

## Omakub
alias cfg='bash ~/.local/share/omakub/bin/omakub'
## Qchao, nice compilation of cpp
alias gpp='function _gpp() {
    if [[ -z "$1" ]]; then
    ...
    g++ -std=c++17 -Wall -Wextra -Werror -Wfatal-errors -pedantic-errors -O2 -o "${1%.cpp}" "$1" && ./"${1%.cpp}"
}; _gpp'

function cs () {
    __zoxide_z "$@" && exa --color=auto --icons --group-directories-first -a
  }
...
alias ls="eza --color=auto --icons -a --group-directories-first"
alias l="ls"
alias la="ls"
alias lla="ls"
alias t="eza --sort name --icons -a --level 2 --group-directories-first --level=2"
alias tree="eza --sort name --icons -a --group-directories-first --tree --level=2"
